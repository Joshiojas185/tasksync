...
app.post("/dev/uploadData", (req, res) => {
...
app.get("/dev/search/user", (req, res) => {
    const { q } = req.query;
...
app.post("/dev/auth", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      user = rows[0];
      await conn.query("UPDATE User SET email = ?, name = ?, picture = ? WHERE googleId = ?", [email, name, picture, googleId]);
    }
    ...
    res.json({
      ...
      user: { id: user.id, googleId: user.googleId, email: user.email, name: user.name, picture: user.picture, role: [user.role] },
      token,
    });
    conn.release();
  } catch (error) {
...
app.post("/api/auth/google", async (req, res) => {
  ...
  try {
    const { tokens } = await client.getToken({ code, redirect_uri: process.env.GOOGLE_REDIRECT_URI || "postmessage" });
    const ticket = await client.verifyIdToken({ idToken: tokens.id_token, audience: process.env.GOOGLE_CLIENT_ID });
    const payload = ticket.getPayload();
    const { sub: googleId, email, name, picture } = payload;

    const [existingUsers] = await pool.query("SELECT * FROM User WHERE googleId = ? OR email = ?", [googleId, email]);
    let user;

    if (existingUsers.length === 0) {
      const [result] = await pool.query(`INSERT INTO User (googleId, email, name, picture, role) VALUES (?, ?, ?, ?, 'user')`, [googleId, email, name, picture]);
      user = { id: result.insertId, googleId, email, name, picture, role: "user" };
    } else {
      user = existingUsers[0];
      await pool.query(`UPDATE User SET name = ?, picture = ? WHERE id = ?`, [name, picture, user.id]);
    }

    const token = generateToken(user);
    // Ensure role is array
    const roleArray = user.role ? [user.role] : [];
    res.json({ success: true, message: "Authentication successful", user: { ...user, role: roleArray }, token });
  } catch (error) {
    console.error("Authentication error:", error);
...
app.post("/api/auth/logout", verifyToken, (req, res) => res.json({ success: true, message: "Logged out successfully" }));

// ==================== USER ROUTES ====================

app.get('/api/users', verifyToken, async (req, res) => {
  try {
    if (!req.user.role.includes('admin')) return res.status(403).json({ success: false, error: 'Admin access required' });
    const [users] = await pool.query(`SELECT id, name, email, picture, role, createdAt, updatedAt FROM User ORDER BY createdAt DESC`);
    const formattedUsers = users.map(user => ({ ...user, role: user.role ? [user.role] : [] }));
...
app.get('/api/users/:id', verifyToken, async (req, res) => {
  try {
    const [users] = await pool.query('SELECT id, name, email, picture, role FROM User WHERE id = ?', [req.params.id]);
...
app.put('/api/users/:id/role', verifyToken, async (req, res) => {
  if (!req.user.role.includes('admin')) return res.status(403).json({ error: 'Admin only' });
  const { role } = req.body;
  const validRoles = ['user', 'admin'];
  if (!role || !validRoles.includes(role)) return res.status(400).json({ success: false, error: `Invalid role: ${validRoles.join(', ')}` });
  try {
    await pool.query('UPDATE User SET role = ? WHERE id = ?', [role, req.params.id]);
...
app.post('/api/boards', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await conn.query('INSERT INTO allowed_users (boardId, userId) VALUES (?, ?)', [result.insertId, req.user.id]);
        conn.release();

        res.status(201).json({ success: true, board: { id: result.insertId, title, description } });
    } catch (err) {
        console.error(err);
...
app.get('/api/boards/:id', verifyToken, async (req, res) => {
    ...
    try {
        const [boards] = await pool.query('SELECT * FROM Board WHERE id = ?', [req.params.id]);
...
app.put('/api/boards/:id', verifyToken, async (req, res) => {
    ...
    const { title, description } = req.body;
    try {
        await pool.query('UPDATE Board SET title = ?, description = ? WHERE id = ?', [title, description, req.params.id]);
...
app.post('/api/boards/:id/users', verifyToken, async (req, res) => {
  ...
  try {
    await pool.query('INSERT IGNORE INTO allowed_users (boardId, userId) VALUES (?, ?)', [req.params.id, userId]);
    res.json({ success: true });
  } catch (err) {
...
app.get('/api/boards/:boardId/epics', verifyToken, async (req, res) => {
    ...
    try {
        const [epics] = await pool.query('SELECT * FROM Epics WHERE boardId = ? ORDER BY createdAt DESC', [req.params.boardId]);
...
app.post('/api/boards/:boardId/epics', verifyToken, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ success: true, epic: { id: result.insertId, title, description } });
    } catch (err) {
        console.error(err);
...
app.get('/api/epics/:id', verifyToken, async (req, res) => {
    try {
        const [epics] = await pool.query('SELECT * FROM Epics WHERE id = ?', [req.params.id]);
        if (epics.length === 0) return res.status(404).json({ error: 'Epic not found' });
...
app.put('/api/epics/:id', verifyToken, async (req, res) => {
    const { title, description } = req.body;
    try {
        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [req.params.id]);
        if (!epics.length) return res.status(404).json({ error: 'Epic not found' });
        
        const hasAccess = await checkBoardAccess(req.user.id, epics[0].boardId);
        if (!hasAccess) return res.status(403).json({ error: 'Access denied' });

        await pool.query('UPDATE Epics SET title = ?, description = ? WHERE id = ?', [title, description, req.params.id]);
        res.json({ success: true });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: 'Update failed' });
...
app.get('/api/epics/:epicId/sprints', verifyToken, async (req, res) => {
    try {
        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [req.params.epicId]);
        if (!epics.length) return res.status(404).json({ error: 'Epic not found' });
        
        const hasAccess = await checkBoardAccess(req.user.id, epics[0].boardId);
        if (!hasAccess) return res.status(403).json({ error: 'Access denied' });

        const [sprints] = await pool.query('SELECT * FROM Sprints WHERE epicId = ? ORDER BY startTime DESC', [req.params.epicId]);
        
        // Parse status JSON for each sprint
        const formattedSprints = sprints.map(sprint => ({
            ...sprint,
            status: sprint.status ? JSON.parse(sprint.status) : []
        }));
...
app.post('/api/epics/:epicId/sprints', verifyToken, async (req, res) => {
    ...
    if (!title || !startTime || !expiry) {
        return res.status(400).json({ error: 'Title, startTime and expiry are required' });
    }
    ...
    try {
        ...
        const [result] = await pool.query(
            ...
            [title, description || null, startTime, expiry, req.params.epicId, req.user.id, sprintStatus]
        );
        ...
        res.status(201).json({ 
            ...
            sprint: { 
                ...
                startTime, 
                expiry, 
                status: JSON.parse(sprintStatus) 
            } 
        });
    } catch (err) {
        console.error(err);
...
app.get('/api/sprints/:id', verifyToken, async (req, res) => {
    try {
        const [sprints] = await pool.query('SELECT * FROM Sprints WHERE id = ?', [req.params.id]);
        if (!sprints.length) return res.status(404).json({ error: 'Sprint not found' });

        const sprint = sprints[0];
        // Parse status JSON
        sprint.status = sprint.status ? JSON.parse(sprint.status) : [];

        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprint.epicId]);
        const hasAccess = await checkBoardAccess(req.user.id, epics[0].boardId);
        if (!hasAccess) return res.status(403).json({ error: 'Access denied' });
...
app.put('/api/sprints/:id', verifyToken, async (req, res) => {
    const { title, description, startTime, expiry, status } = req.body;
    try {
        const [sprints] = await pool.query('SELECT epicId FROM Sprints WHERE id = ?', [req.params.id]);
        if (!sprints.length) return res.status(404).json({ error: 'Sprint not found' });

        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
        const hasAccess = await checkBoardAccess(req.user.id, epics[0].boardId);
        if (!hasAccess) return res.status(403).json({ error: 'Access denied' });
...
app.get('/api/sprints/:sprintId/tasks', verifyToken, async (req, res) => {
    try {
        const [sprints] = await pool.query('SELECT epicId, status FROM Sprints WHERE id = ?', [req.params.sprintId]);
        if (!sprints.length) return res.status(404).json({ error: 'Sprint not found' });
        ...
        // Parse sprint status to get available statuses
        const sprintStatus = sprints[0].status ? JSON.parse(sprints[0].status) : [];
...
app.post('/api/sprints/:sprintId/tasks', verifyToken, async (req, res) => {
    const { title, description, assigneeId, storyPoints, hours, priority, status } = req.body;
    ...
    try {
        ...
        const sprintStatusArray = sprints[0].status ? JSON.parse(sprints[0].status) : ['To Do'];
        ...
        const [result] = await pool.query(
            `INSERT INTO Tasks (sprintId, title, description, assigneeId, storyPoints, hours, status, priority, createdBy) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [req.params.sprintId, title, description || null, assigneeId || null, storyPoints || 0, hours || 0, taskStatus, priority || 'Medium', req.user.id]
        );

        res.status(201).json({ success: true, task: { id: result.insertId, title, status: taskStatus } });
    } catch (err) {
        console.error(err);
...
app.get('/api/tasks/:id', verifyToken, async (req, res) => {
    try {
        ...
        const [sprints] = await pool.query('SELECT id, epicId, status FROM Sprints WHERE id = ?', [task.sprintId]);
        const sprintStatus = sprints[0].status ? JSON.parse(sprints[0].status) : [];
...
app.put('/api/tasks/:id', verifyToken, async (req, res) => {
    const { title, description, assigneeId, storyPoints, hours, priority, status } = req.body;
    try {
        const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [req.params.id]);
        if (!tasks.length) return res.status(404).json({ error: 'Task not found' });

        const [sprints] = await pool.query('SELECT epicId, status FROM Sprints WHERE id = ?', [tasks[0].sprintId]);
        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
        ...
        if (status) {
            const sprintStatusArray = sprints[0].status ? JSON.parse(sprints[0].status) : [];
            if (!sprintStatusArray.includes(status)) {
                return res.status(400).json({ error: `Invalid status. Available: ${sprintStatusArray.join(', ')}` });
            }
        }
        ...
        await pool.query(
            `UPDATE Tasks SET title = ?, description = ?, assigneeId = ?, storyPoints = ?, hours = ?, priority = ?, status = COALESCE(?, status) 
             WHERE id = ?`,
            [title, description, assigneeId, storyPoints, hours, priority, status, req.params.id]
        );
        res.json({ success: true });
    } catch (err) {
        console.error(err);
...
app.delete('/api/tasks/:id', verifyToken, async (req, res) => {
    try {
        ...
        const [sprints] = await pool.query('SELECT epicId FROM Sprints WHERE id = ?', [tasks[0].sprintId]);
        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
...
app.get('/api/tasks/:taskId/subtasks', verifyToken, async (req, res) => {
    try {
        const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [req.params.taskId]);
        if (!tasks.length) return res.status(404).json({ error: 'Task not found' });
...
app.post('/api/tasks/:taskId/subtasks', verifyToken, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ success: true, subtask: { id: result.insertId, title, description, hours: hours || 0 } });
    } catch (err) {
        console.error(err);
...
app.put('/api/subtasks/:id', verifyToken, async (req, res) => {
    ...
    try {
        const [subtasks] = await pool.query('SELECT * FROM Subtasks WHERE id = ?', [req.params.id]);
        if (!subtasks.length) return res.status(404).json({ error: 'Subtask not found' });

        const sub = subtasks[0];
        
        const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [sub.taskId]);
        const [sprints] = await pool.query('SELECT epicId FROM Sprints WHERE id = ?', [tasks[0].sprintId]);
        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
...
app.delete('/api/subtasks/:id', verifyToken, async (req, res) => {
    try {
        const [subtasks] = await pool.query('SELECT taskId FROM Subtasks WHERE id = ?', [req.params.id]);
        if (!subtasks.length) return res.status(404).json({ error: 'Subtask not found' });

        const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [subtasks[0].taskId]);
        const [sprints] = await pool.query('SELECT epicId FROM Sprints WHERE id = ?', [tasks[0].sprintId]);
        const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
...
app.get('/api/subtasks/:subtaskId/comments', verifyToken, async (req, res) => {
    try {
        const [subtasks] = await pool.query('SELECT taskId FROM Subtasks WHERE id = ?', [req.params.subtaskId]);
        if (!subtasks.length) return res.status(404).json({ error: 'Subtask not found' });
...
app.delete('/api/comments/:id', verifyToken, async (req, res) => {
    try {
        ...
        let boardId;
        if (comment.taskId) {
            const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [comment.taskId]);
            const [sprints] = await pool.query('SELECT epicId FROM Sprints WHERE id = ?', [tasks[0].sprintId]);
            const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
            boardId = epics[0].boardId;
        } else if (comment.subtaskId) {
            const [subtasks] = await pool.query('SELECT taskId FROM Subtasks WHERE id = ?', [comment.subtaskId]);
            const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [subtasks[0].taskId]);
            const [sprints] = await pool.query('SELECT epicId FROM Sprints WHERE id = ?', [tasks[0].sprintId]);
            const [epics] = await pool.query('SELECT boardId FROM Epics WHERE id = ?', [sprints[0].epicId]);
            boardId = epics[0].boardId;
...
app.get('/api/timesheet', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const totalHours = timesheet.reduce((sum, t) => sum + (parseFloat(t.hoursWorked) || 0), 0);
...
app.get('/api/subtasks/:subtaskId/worklogs', verifyToken, async (req, res) => {
    try {
        const [subtasks] = await pool.query('SELECT taskId, hours FROM Subtasks WHERE id = ?', [req.params.subtaskId]);
        if (!subtasks.length) return res.status(404).json({ error: 'Subtask not found' });
        ...
        const totalLoggedHours = worklogs.reduce((sum, log) => sum + parseFloat(log.hoursWorked || 0), 0);
        
        res.json({ 
            success: true, 
            subtaskHours: parseFloat(subtasks[0].hours) || 0,
            totalLoggedHours: parseFloat(totalLoggedHours.toFixed(2)),
            remainingHours: parseFloat((subtasks[0].hours - totalLoggedHours).toFixed(2)),
            worklogs 
        });
    } catch (err) {
        console.error(err);
...
//         return res.status(400).json({ error: 'workDate and hoursWorked are required' });
//     }
...
//             [req.params.subtaskId, req.user.id, workDate, hoursWorked]
//         );
...
app.post('/api/subtasks/:subtaskId/worklogs', verifyToken, async (req, res) => {
    ...
    try {
        const [subtasks] = await pool.query('SELECT taskId, hours, assigneeId FROM Subtasks WHERE id = ?', [req.params.subtaskId]);
        if (!subtasks.length) return res.status(404).json({ error: 'Subtask not found' });
...
app.post('/api/subtasks/:subtaskId/worklogs/bulk', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const currentTotal = parseFloat(existingLogs[0].totalHours || 0);
        const newLogsTotal = workLogs.reduce((sum, log) => sum + parseFloat(log.hoursWorked || 0), 0);
...
app.delete('/api/worklogs/:id', verifyToken, async (req, res) => {
    try {
        const [worklogs] = await pool.query('SELECT * FROM SubtaskWorkLogs WHERE id = ?', [req.params.id]);
        if (!worklogs.length) return res.status(404).json({ error: 'Work log not found' });

        // Only the user who created the log or admin can delete
        if (worklogs[0].userId !== req.user.id && !req.user.role.includes('admin')) {
...
app.post('/api/comments', verifyToken, async (req, res) => {
    ...
    if (!content) return res.status(400).json({ error: 'Content is required' });
    if (!taskId && !subtaskId) return res.status(400).json({ error: 'Either taskId or subtaskId is required' });
    if (taskId && subtaskId) return res.status(400).json({ error: 'Provide only taskId OR subtaskId, not both' });
    ...
    try {
        ...
        if (taskId) {
            // Get board from task
            const [tasks] = await pool.query('SELECT sprintId FROM Tasks WHERE id = ?', [taskId]);
            if (!tasks.length) return res.status(404).json({ error: 'Task not found' });
            ...
            const [subtasks] = await pool.query('SELECT taskId FROM Subtasks WHERE id = ?', [subtaskId]);
            if (!subtasks.length) return res.status(404).json({ error: 'Subtask not found' });
